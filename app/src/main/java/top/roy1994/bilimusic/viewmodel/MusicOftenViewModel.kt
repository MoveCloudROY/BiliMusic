...
class MusicOftenViewModel(application: Application): AndroidViewModel(application) {
    ...
    val musicOften: LiveData<List<MusicEntity>> = musicDao.loadMusicsPlayTimesDesc()

}

class MusicOftenViewModelFactory(
    private val application: Application
) : ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        @Suppress("UNCHECKED_CAST")
        if (modelClass.isAssignableFrom(MusicOftenViewModel::class.java)) {
            return MusicOftenViewModel(application) as T
        }
